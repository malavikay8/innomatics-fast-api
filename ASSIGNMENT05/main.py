...
def sort_products(sort_by: str = "price", order: str = "asc"):
    ...
    # Only allow sorting by 'price' or 'name'
    if sort_by not in ["price", "name"]:
        return {"error": "sort_by must be 'price' or 'name'"}

    # 🔃 Sort the products list based on selected field
    # lambda function gets the value of sort_by field
    sorted_products = sorted(products, key=lambda x: x.get(sort_by))
...
def get_products_page(page: int = 1, limit: int = 2):
    ...
    # 📤 Return response with extra info
    return {
        "page": page,
        "limit": limit,
...
def create_order(order: Order):
    new_order = {
        ...
        "customer_name": order.customer_name,   # ✅ ADD THIS
        "product_id": order.product_id,
        "quantity": order.quantity,
        "status": "pending"
    }
...
def sort_by_category():
    ...
    # First by category (alphabetical), then by price (ascending)
